...
class LGBMClassifier:
    def __init__(self):
        self.features = ['delta_proportion', 'theta_proportion', 'alpha_proportion', 
    ...
    def prepare_data(self, data, feature, state):
        # 筛选特定状态的数据
        state_data = data[data['state'] == state]
        
        # 准备特征数据：每个filename为一个样本，每个通道为一个特征
        X = state_data.pivot(index='filename', columns='ch_name', values=feature)
    ...
    def train_and_evaluate(self, X, y):
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
            ...
            model = lgb.LGBMClassifier(
                ...
                random_state=42,
                min_child_samples=3,  # 减小每个叶子节点最小样本数
                num_leaves=15,        # 叶子节点数量
                max_depth=4         # 不限制树的深度
            )
            
            
            callbacks = [
                lgb.callback.log_evaluation(period=10),  # 每10次迭代显示一次结果
                lgb.callback.early_stopping(stopping_rounds=30)  # 20轮无提升就早停
            ]
    ...
    def plot_roc_curves(self, state):
        plt.figure(figsize=(20, 15))
        for i, feature in enumerate(self.features, 1):
            plt.subplot(4, 4, i)
            for result in self.results:
                if result['status'] == state and result['feature'] == feature:
        ...
        plt.suptitle(f'ROC Curves for {state}', y=1.02, fontsize=16)
        plt.tight_layout()
        plt.savefig(f'./save_jpg/ROC/{state}_Classifier_ROC_AUC.jpg', 
                    bbox_inches='tight', dpi=300)
        plt.close()
    
    def save_results_csv(self):
        df_results = pd.DataFrame(self.results)
        df_results = df_results[['group', 'feature', 'status', 'AUC']]
        df_results.to_csv('./save_csv/ROC/POD-noPOD_Classifier_AUC_results.csv', 
                         index=False)
    
    def run(self):
        # 读取数据
        data = pd.read_csv('save_csv/data/process_filename_frequency_distribution_median.tsv', 
                          sep='\t')
        ...
        for state in self.states:
            for feature in self.features:
                ...
                # 准备数据
                X, y = self.prepare_data(data, feature, state)
                ...
                # 保存模型
                self.save_model_onnx(best_model, state, feature)
        
        # 为每个状态绘制ROC曲线
        for state in self.states:
            self.plot_roc_curves(state)
